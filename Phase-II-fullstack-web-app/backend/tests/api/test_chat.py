...
class TestChatRateLimiting:
    ...
    @patch('src.api.routes.chat.create_agent_orchestrator')
    @patch('src.api.routes.chat.limiter')
    def test_ta012_chat_endpoint_returns_429_when_rate_limit_exceeded(
        self, mock_limiter, mock_orchestrator_factory, test_client_with_db
    ):
        """TA012: Test chat endpoint returns 429 when rate limit exceeded.

        Note: This is a conceptual test. Actual rate limit exhaustion would
        require 60+ requests which is slow for unit tests. The test verifies
        the error structure is correct.
        """
        from slowapi.errors import RateLimitExceeded
        from slowapi import Limit

        # Create a mock Limit object
        mock_limit = Limit("60/minute", lambda: "60/minute")

        # Mock the rate limiter to raise RateLimitExceeded
        mock_limiter.limit.return_value = lambda f: f  # Passthrough decorator
        mock_limiter.check.side_effect = RateLimitExceeded(mock_limit)
        ...
        # Request should trigger rate limit (429)
        response = client.post(
            f"/api/{user_id}/chat",
